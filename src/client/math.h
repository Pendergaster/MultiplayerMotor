...
struct quaternion
{
	...
	quaternion(vec3 v,float theata ) // axis angle rotation
	{
		normalize(&v);
#if 0
	...
	void operator *=(const quaternion& rhv)  {
		// TODO(pate) kato saakko järkevämmäksi
		*this = *this * rhv;
	}
	quaternion operator *(const quaternion& rhv) const
	{
		...
		scalar * rhv.scalar -	i *			rhv.i -			j *		rhv.j - k *			rhv.k,
			   scalar * rhv.i +		i *			rhv.scalar +	j *		rhv.k - k *			rhv.j,
			   scalar * rhv.j +		j *			rhv.scalar +	k *		rhv.i - i *			rhv.k,
			   scalar * rhv.k +		k *			rhv.scalar +	i *		rhv.j - j *			rhv.i
				   );
#endif
#endif
	}
...
static void scale(mat4* mat,float scale)
{
	for (int x = 0; x < 3; x++)
		for (int y = 0; y < 3; y++)
			mat->mat[x][y] *= scale;		
...
static inline void inverse_mat4(mat4* res, mat4* m)
	//assumes that matrix is invertable, implementation similar to linmath and glu
{
	float s[6];
	float c[6];
